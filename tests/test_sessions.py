...
def test_coverage_is_the_time_within_reach_of_a_cycle() -> None:
    start, end = utc(MON, 13, 30), utc(MON, 14, 30)
...
def test_no_cycles_covers_nothing() -> None:
    start, end = utc(MON, 13, 30), utc(MON, 20)
...
def test_a_session_the_loop_ran_through_is_clean(book: Ledger, clock: PinnedClock) -> None:
    run(book, clock, run_id="run_a")

    record = _read(book, now=utc(MON, 21))
...
def test_a_session_is_not_judged_until_the_close_and_the_silence_bound_pass(
    ...
    for now in (utc(MON, 15), utc(MON, 20, 5), utc(MON, 20, 14)):
    ...
    judged = _only(_read(book, now=utc(MON, 20, 16)))
...
def test_a_run_still_cycling_after_the_close_leaves_a_clean_session(
    ...
    run(book, clock, run_id="run_a", stop=(23, 50), end=None)

    record = _read(book, now=utc(MON, 23, 55))
    session = _only(record)
    assert session.session_date == MON
    assert session.verdict is SessionVerdict.CLEAN, session.summary()
...
def test_stopping_the_loop_after_the_close_does_not_open_the_next_session(
    ...
    run(book, clock, run_id="run_a", stop=(20, 20), end="interrupted")

    record = _read(book, now=utc(TUE, 22))
    session = _only(record)
    assert session.session_date == MON
...
def test_a_late_start_is_incomplete_not_clean(book: Ledger, clock: PinnedClock) -> None:
    """Noon to the close is 61.5% of the session: the loop did not run it."""
    run(book, clock, run_id="run_a", start=(16, 0))

    session = _only(_read(book, now=utc(MON, 21)))
...
def test_a_short_gap_is_tolerated_and_a_long_one_is_not(
    ledger_path: Path, clock: PinnedClock
) -> None:
    """Twenty minutes without a cycle loses five; an hour and a half loses enough."""
    short = (utc(MON, 16), utc(MON, 16, 20))
    with Ledger(ledger_path) as book:
        book.initialise(created_by="test")
        run(book, clock, run_id="run_a", skip=short)
        session = _only(_read(book, now=utc(MON, 21)))
    assert session.verdict is SessionVerdict.CLEAN
    # Cycles at 15:50 and 16:20: the one at 15:50 reaches 16:05.
    assert session.longest_gap_seconds == 15 * 60

    long = (utc(TUE, 16), utc(TUE, 17, 30))
    with Ledger(ledger_path) as book:
        run(book, clock, run_id="run_b", day=TUE, skip=long)
        tuesday = _read(book, now=utc(TUE, 21)).session(TUE)
...
def test_a_halted_run_faults_its_session(book: Ledger, clock: PinnedClock) -> None:
    run(
        book,
        clock,
        run_id="run_a",
...
def test_an_error_ending_faults_its_session(book: Ledger, clock: PinnedClock) -> None:
    run(
        book,
        clock,
        run_id="run_a",
...
def test_an_operator_stop_is_a_note_and_the_coverage_decides(
    book: Ledger, clock: PinnedClock
) -> None:
    """Ctrl-C is not a fault; stopping at two o'clock still did not run the session."""
    run(book, clock, run_id="run_a", stop=(18, 0), end="interrupted")
...
def test_the_kill_switch_counts_only_while_a_run_is_up(book: Ledger, clock: PinnedClock) -> None:
    ...
    append(book, clock, utc(MON, 15), EventType.KILLSWITCH_ENGAGED, engaged)
    append(book, clock, utc(TUE, 22), EventType.KILLSWITCH_ENGAGED, engaged)

    record = _read(book, now=utc(WED, 12))
...
def test_a_manual_halt_and_a_watchdog_trip_while_up_are_faults(
    book: Ledger, clock: PinnedClock
) -> None:
...
def test_a_wedged_loop_owns_the_watchdog_trip_its_silence_caused(
    ...
    last = run(book, clock, run_id="run_a", stop=(15, 0), end=None)
    append(
        book,
        clock,
...
def test_a_withdrawn_stop_is_not_a_fault_and_a_failed_one_is(
    book: Ledger, clock: PinnedClock, cause: str | None, detail: str, faulted: bool
) -> None:
    run(book, clock, run_id="run_a")
    append(
        book,
...
def test_an_entry_left_bare_too_long_faults_its_session(book: Ledger, clock: PinnedClock) -> None:
    run(book, clock, run_id="run_a")
    run(book, clock, run_id="run_b", day=TUE)
    ...
    monday, tuesday = _read(book, now=utc(WED, 12)).sessions
    assert monday.verdict is SessionVerdict.CLEAN
    assert _kinds(tuesday.faults) == [FaultKind.UNPROTECTED_TOO_LONG]


def _commit(ledger: Ledger, clock: PinnedClock, at: datetime, intent_id: str) -> None:
    append(
        ledger,
        clock,
        at,
...
def _ack(ledger: Ledger, clock: PinnedClock, at: datetime, intent_id: str) -> None:
    append(
        ledger,
        clock,
        at,
...
def test_an_order_left_in_an_unknown_state_faults_its_session(
    book: Ledger, clock: PinnedClock
) -> None:
    """Answered at once is fine; answered after twenty minutes, or never, is not."""
    run(book, clock, run_id="run_a")
    _commit(book, clock, utc(MON, 14), "int_prompt")
    _ack(book, clock, utc(MON, 14) + timedelta(seconds=1), "int_prompt")
    _commit(book, clock, utc(MON, 15), "int_slow")
    _ack(book, clock, utc(MON, 15, 20), "int_slow")
    _commit(book, clock, utc(MON, 16), "int_never")
...
def test_an_order_just_sent_is_not_yet_unknown(book: Ledger, clock: PinnedClock) -> None:
    run(book, clock, run_id="run_a", end=None, stop=(16, 0))
    _commit(book, clock, utc(MON, 16, 1), "int_in_flight")

    session = _only(_read(book, now=utc(MON, 16, 2)))
    assert not session.faults
...
def test_modes_are_counted_apart(book: Ledger, clock: PinnedClock) -> None:
    """Thirty clean days on the simulated broker say nothing about Trading 212."""
    run(book, clock, run_id="run_paper", mode="paper", day=MON)
    run(book, clock, run_id="run_demo", mode="demo", day=TUE, stop=(19, 0), end="halted")

    paper = _read(book, now=utc(WED, 12), mode="paper")
    demo = _read(book, now=utc(WED, 12), mode="demo")
    ...
    assert _read(book, now=utc(WED, 12), mode="live").sessions == ()


def test_a_crash_over_the_weekend_breaks_monday(book: Ledger, clock: PinnedClock) -> None:
    """The run was waiting for Monday; its death belongs to Monday, not to nobody."""
    run(book, clock, run_id="run_a", day=FRI, stop=(23, 50), end=None)
    cycle(book, clock, "run_a", utc(SAT, 10), 99)

    record = _read(book, now=utc(NEXT_MON, 21))
...
def test_notes_do_not_make_a_session_unclean(book: Ledger, clock: PinnedClock) -> None:
    run(book, clock, run_id="run_a")
    append(
        book,
        clock,
    ...
    cycle(book, clock, "run_a", utc(MON, 17, 5), 100, detail="settlement deferred: 503")
...
def test_activity_is_counted_per_session(book: Ledger, clock: PinnedClock) -> None:
    run(book, clock, run_id="run_a", stop=(14, 0))
    run(book, clock, run_id="run_b", start=(14, 10))
    cycle(book, clock, "run_b", utc(MON, 19, 55), 50, decisions=2, orders=1, fills=1)
    append(
        book,
        clock,
        utc(MON, 19),
...
def test_the_streak_counts_back_to_the_first_unclean_session(
    book: Ledger, clock: PinnedClock
) -> None:
    """A fault resets it; a session still being judged neither counts nor breaks it."""
    run(book, clock, run_id="run_mon", day=MON)
    run(book, clock, run_id="run_tue", day=TUE, stop=(19, 0), end="halted")
    run(book, clock, run_id="run_wed", day=WED)
    run(book, clock, run_id="run_thu", day=THU)
    run(book, clock, run_id="run_fri", day=FRI, stop=(15, 0), end=None)

    record = _read(book, now=utc(FRI, 15, 5))
...
def test_a_day_without_a_run_neither_counts_nor_breaks_the_streak(
    ...
    record = _read(book, now=utc(FRI, 12))
...
def test_the_reader_refuses_an_unknown_mode(book: Ledger) -> None:
    with pytest.raises(ValueError, match="no such trading mode"):
        read_sessions(book, limits=LIVE, mode="research", now=utc(MON, 12))
...
def test_tb_sessions_lists_the_sessions_and_the_streak_the_gate_counts(
    ...
    with Ledger(ledger_path) as book:
        ...
        run(book, clock, run_id="run_wed", day=WED)
    monkeypatch.setattr("tb.ops.sessions.now_utc", lambda: utc(THU, 12))
